    ...
    try {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['name']) {
            ...
            if (!in_array($imageExtension, $validImageExtensions)) {
                ...
                $newImageName = $customerID . '_' . date("Y.m.d") . '_' . date("h.i.sa") . '.' . $imageExtension;

                // Save the image to your server
                $uploadPath = $_SERVER['DOCUMENT_ROOT'] . '/nasara/images/' . $newImageName;
                move_uploaded_file($tmpName, $uploadPath);

                // Update the customer's profile picture in tbl_customer_info
                $updateProfilePic = $conn->prepare("UPDATE tbl_customer_info SET image = :image WHERE customer_ID = :customerID");
                $updateProfilePic->bindParam(':image', $newImageName);
                $updateProfilePic->bindParam(':customerID', $customerID);
        ...
        $insertActivity->execute(["Changed Profile Picture", $customerID]);

        // // Redirect to the user's profile page
        // echo '<script>alert("Profile Picture Successfully Changed!");';
        // echo 'window.location.href = "http://localhostnasara/account_main.php";</script>';
